...
class ProductManager {
    ...
    async updateProduct(id, changeProperty, newValue) {
        try {
            ...
            const index = await allproducts.findIndex(p => p.id === productUpdate.id)
    ...
    async deleteProduct(id) {
        try {
            ...
            const index = await allproducts.findIndex(p => p.id === deleteProduct.id)
            allproducts.splice(index, 1)
            await fs.promises.writeFile(this.path, JSON.stringify(allproducts, JSON.stringify(allproducts)))
        } catch (e) {
            console.log(e);
        }
    }
